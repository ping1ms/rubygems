...
RSpec.describe "compact index api" do
  ...
  it "falls back when the versions endpoint has a checksum mismatch" do
    ...
    expect(out).to include('Local checksums {"sha-256"=>"ungWv48Bz+pBQUDeXa4iI7ADYaOWF3qctBD/YfIAFa0="} did not match {"sha-256"=>"8KfZiM/fszVkqhP/m5s9lvE6M9xKu4I1bU4Izddp5Ms="}')
    expect(the_bundle).to include_gems "rack 1.0.0"
  end
